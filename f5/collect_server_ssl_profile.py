from f5 import get_list_server_ssl_profile
...
def update_server_ssl_profile(datalist):
    api = 'api/cm/f5/update-server-ssl-profile'
    url = icms_base_url + api
    ...
    if get_list_f5_device_result['status'] == 'success':
        ...
        for f5_device_ip in list_f5_device_ip:
            ...
            if get_session_token_result['status'] == 'success':
                ...
                get_list_server_ssl_profile_result = get_list_server_ssl_profile(
                    base_url=base_url,
                    token=token
                )
                if get_list_server_ssl_profile_result['status'] == 'success':
                    list_server_ssl_profile = get_list_server_ssl_profile_result['datalist']
                    update_data = dict()
                    update_data[f5_device_ip] = list_server_ssl_profile
                    update_server_ssl_profile_result = update_server_ssl_profile(update_data)
                    if update_server_ssl_profile_result['status'] == 'success':
                        print('Update server SSL profile to ICMS success')
                    else:
                        error = update_server_ssl_profile_result['error']
                        print(f'Update server SSL profile to ICMS failed - error: {error}')
            else:
