from f5 import get_list_client_ssl_profile
...
def update_client_ssl_profile(datalist):
    api = 'api/cm/f5/update-client-ssl-profile'
    url = icms_base_url + api
    ...
    if get_list_f5_device_result['status'] == 'success':
        ...
        for f5_device_ip in list_f5_device_ip:
            ...
            if get_session_token_result['status'] == 'success':
                ...
                get_list_client_ssl_profile_result = get_list_client_ssl_profile(
                    base_url=base_url,
                    token=token
                )
                if get_list_client_ssl_profile_result['status'] == 'success':
                    list_client_ssl_profile = get_list_client_ssl_profile_result['datalist']
                    update_data = dict()
                    update_data[f5_device_ip] = list_client_ssl_profile
                    update_client_ssl_profile_result = update_client_ssl_profile(update_data)
                    if update_client_ssl_profile_result['status'] == 'success':
                        print('Update client SSL profile to ICMS success')
                    else:
                        error = update_client_ssl_profile_result['error']
                        print(f'Update client SSL profile to ICMS failed - error: {error}')
            else:
