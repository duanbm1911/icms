from f5 import get_list_waf_profile
...
def update_waf_profile(datalist):
    api = 'api/cm/f5/update-waf-profile'
    url = icms_base_url + api
    ...
    if get_list_f5_device_result['status'] == 'success':
        ...
        for f5_device_ip in list_f5_device_ip:
            ...
            if get_session_token_result['status'] == 'success':
                ...
                get_list_waf_profile_result = get_list_waf_profile(
                    base_url=base_url,
                    token=token
                )
                if get_list_waf_profile_result['status'] == 'success':
                    list_waf_profile = get_list_waf_profile_result['datalist']
                    update_data = dict()
                    update_data[f5_device_ip] = list_waf_profile
                    update_waf_profile_result = update_waf_profile(update_data)
                    if update_waf_profile_result['status'] == 'success':
                        print('Update server waf to ICMS success')
                    else:
                        error = update_waf_profile_result['error']
                        print(f'Update server waf to ICMS failed - error: {error}')
            else:
